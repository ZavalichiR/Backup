	...
	public void StaffHandler.showWaitingList(){
		if(waitingReservation.isEmpty()==true){
			...
			for( Reservation rst : getWaitingList()){
				System.out.println("Clientul cu id: "+rst.getCustomer().getId()+" doreste sa rezerve camera "+rst.getRoom().getCateg().toString()+" pentru "+rst.getNrOfDays()+" zile.");
